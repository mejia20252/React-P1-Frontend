...
// El nombre del componente se mantiene como MisCuotas, ya que la lógica es genérica.
// Si este componente será EXCLUSIVAMENTE para inquilinos, podrías renombrarlo a MisCuotasInquilino.
const MisCuotas: React.FC = () => {
    ...
    useEffect(() => {
        const loadCuotas = async () => {
            try {
                setLoading(true);
                // La ruta '/cuotas/mis-cuotas/' debería ser manejada por el backend
                // para devolver las cuotas correspondientes al usuario autenticado,
                // ya sea propietario o inquilino.
                const { data } = await api.get('/cuotas/mis-cuotas/');
                // Asume que el backend devuelve un array de cuotas directamente o dentro de 'results'
    ...
    const formatDate = (dateString: string | null): string => {
        ...
        // Ajusta la zona horaria si es necesario, de lo contrario, puede haber desfases
    ...
    const formatCurrency = (amount: string): string => {
        ...
        return new Intl.NumberFormat('es-BO', {
            style: 'currency',
            currency: 'USD', // Asegúrate de que esta sea la divisa correcta para tus cuotas
    ...
    const handlePagarConStripe = async (cuotaId: number) => {
        ...
        try {
            ...
            const response = await api.post(
                '/pagos/crear_sesion_stripe/', // URL de tu API para crear la sesión de Stripe
                {
                    tipo_objeto: 'cuota',
                    objeto_id: cuotaId,
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4">
            <div className="max-w-4xl mx-auto">
                {/* El título puede ser "Mis Cuotas" para ambos roles si es genérico */}
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            Propiedad {/* Cambiado de 'Casa' a 'Propiedad' para ser más genérico/claro para inquilinos */}
                                        </th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            Acción
                                        </th>
                                    ...
                                    {cuotas.map((cuota) => (
                                        <tr key={cuota.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {/* Asumiendo que `cuota.casa` es el ID o algún identificador de la propiedad */}
                                                #{cuota.casa}
                                            </td>
