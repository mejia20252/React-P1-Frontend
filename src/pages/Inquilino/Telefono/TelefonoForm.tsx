...
const TelefonoForm: React.FC = () => {
  ...
  const [telefonoNotFound, setTelefonoNotFound] = useState(false); // New state for not found
  ...
  useEffect(() => {
    const loadTelefono = async () => {
      ...
      setLoading(true);
      setTelefonoNotFound(false); // Reset not found status
      try {
        ...
        if (data) {
          ...
          setTelefonoNotFound(true); // Set not found if data is null or undefined
          setTopError('No se encontró el teléfono.');
        }
      } catch (err) {
        setTopError('No se pudo cargar el teléfono.');
        setTelefonoNotFound(true); // Also set not found on API error
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (values: TelefonoFormState) => {
    ...
    try {
      ...
      navigate('/inquilino/telefonos'); // Changed from propietario to inquilino
    } catch (error) {
      console.log('🚀 errr'); 
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-50 p-4">
      <div className="w-full max-w-lg bg-white rounded-lg shadow-xl p-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => navigate('/inquilino/telefonos')} // Changed from propietario to inquilino
        ...
        ) : telefonoNotFound ? ( // Conditionally render "no se encontró" message
          <div className="flex justify-center items-center min-h-[150px] flex-col">
            <p className="text-gray-600 text-lg mb-4">No se encontró el teléfono solicitado.</p>
            <button
              onClick={() => navigate('/inquilino/telefonos')} // Changed from propietario to inquilino
              className="flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-blue-100 rounded-md hover:bg-blue-200 transition-colors duration-200"
            >
